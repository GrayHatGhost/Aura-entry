...
export default function Home() {
  ...
  const activeNodes = useMemo(
    () => (experience?.nodes ?? []).filter((node) => node.is_active).sort((a, b) => a.sort_order - b.sort_order),
    [experience]
  )

  const rootNodeId = useMemo(() => {
    if (!experience) return ''
    const configuredRoot = experience.settings.root_node_id
    if (activeNodes.some((node) => node.id === configuredRoot)) return configuredRoot
    return activeNodes[0]?.id ?? ''
  }, [activeNodes, experience])

  const currentNode = useMemo(
    () =>
      activeNodes.find((node) => node.id === currentNodeId) ??
      activeNodes.find((node) => node.id === rootNodeId) ??
      activeNodes[0] ??
      null,
    [activeNodes, currentNodeId, rootNodeId]
  )
  ...
  const currentButtons = useMemo(
    () =>
      (experience?.buttons ?? [])
        .filter((button) => button.node_id === currentNode?.id && button.is_active)
  ...
  const openNode = useCallback((targetNodeId: string | null) => {
    ...
    const activeCurrentId = currentNode?.id ?? currentNodeId
    let targetNode = targetNodeId ? activeNodes.find((node) => node.id === targetNodeId) ?? null : null

    // Admin panelde hedef kapı boş bırakıldıysa veya yanlışlıkla mevcut kapıya bağlandıysa
    // kullanıcıyı ölü bir geçişte bırakmak yerine sıradaki aktif kapıya geçir.
    if (!targetNode || targetNode.id === activeCurrentId) {
      const currentIndex = activeNodes.findIndex((node) => node.id === activeCurrentId)
      const fallbackNodes = activeNodes.filter((node) => node.id !== activeCurrentId)

      if (!fallbackNodes.length) return false

      targetNode =
        currentIndex >= 0
          ? activeNodes.slice(currentIndex + 1).find((node) => node.id !== activeCurrentId) ??
            activeNodes.slice(0, currentIndex).find((node) => node.id !== activeCurrentId) ??
            fallbackNodes[0]
          : fallbackNodes[0]
    }

    if (!targetNode || targetNode.id === activeCurrentId) return false

    setHistory((prev) => (activeCurrentId ? [...prev, activeCurrentId] : prev))
    ...
    return true
  }, [activeNodes, currentNode?.id, currentNodeId])
